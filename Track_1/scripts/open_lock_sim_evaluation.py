...
EVAL_CFG_FILE = os.path.join(track_path, "configs/evaluation/open_lock_evaluation.yaml")
KEY_NUM = 4
REPEAT_NUM = 2
...
def evaluate_policy(model, key):
    ...
    offset_list = [[i * 1.0 / 2 - 5, 0, 0] for i in range(20)]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--team_name", type=str, required=True, help="Enter your team's name."
    )
    parser.add_argument(
        "--model_name", required=True, help="Enter the name of your modified model."
    )
    parser.add_argument(
        "--policy_file_path",
        required=True,
        help="Enter the path of your trained best_model.",
    )
